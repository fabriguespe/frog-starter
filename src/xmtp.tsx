...
export const xmtpSupport = async (c: Context, next: Next) => {
  ...
  const newMetaTag = '<meta property="of:accepts:xmtp" content="vNext" />';
  // Insert the new meta tag after the opening <head> tag
  html = html.replace(/(<head[^>]*>)/i, `$1${newMetaTag}`);
...
export const validateXMTPUser = async (c: Context, next: Next) => {
  ...
  console.log("entrando a validateXMTPUser", requestBody);
